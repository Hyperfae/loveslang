...
while [ "$#" -gt 0 ]; do
    ...
    if [[ "$1" = "VERTEX="* ]]; then
        entrypoint="${1#"VERTEX="}"
        echo "#ifdef VERTEX"
        slangc -I"$loveslang_path/src/slang" -DVARYING=out -DLOVESLANG=LOVESLANG -target glsl "$input" -entry "$entrypoint" \
            | applyCommonFixups \
        ...
        entrypoint="${1#"PIXEL="}"
        echo "#ifdef PIXEL"
        slangc -I"$loveslang_path/src/slang" -DVARYING=in -DLOVESLANG=LOVESLANG -target glsl "$input" -entry "$entrypoint" \
            | applyCommonFixups \
